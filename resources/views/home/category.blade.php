                        ...
                        @foreach ($projects as $item)
                            <div class="col-lg-4 col-md-6 offset-md-3 offset-lg-0">
                                <div class="property-card">
                                    ...
                                    badge-secondary
                                    @elseif($item->status == 'غير متاح للعرض')
                                    badge-danger
                                    @else
                                    badge-warning text-white @endif"
                                        style="padding: 5px 15px; font-size: 14px;  top: 26px;
                                    position: relative;">{{ $item->status }}
                                        @if ($item->status == 'على وشك الانتهاء' || $item->status == 'غير متاح للعرض')
                                            <i class="bx bx-lock"></i>
                                        @endif
                                    </span>


                                    <a @if ($item->status != 'غير متاح للعرض') href="{{ route('project', $item->id) }}" @endif>
                                    ...
                                    <div class="content">
                                        ...
                                        @if ($item->status != 'غير متاح للعرض')
                                        <a href="{{ route('project', $item->id) }}" class="learn-more-btn">
                                            <i class='bx bx-right-arrow-alt'></i>
                                            تصفح المشروع
                                        </a>
                                        @else
                                        <a  class="learn-more-btn text-danger">لايمكن تصفح المشروع بعد</a>
                                        @endif
                                    </div>
                                </div>
                            </div>
